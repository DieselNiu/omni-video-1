...
type Props = Omit<ComponentProps<typeof Turnstile>, 'siteKey'> & {
  ...
  /**
   * Optional override for the site key. Prefer passing this from the
   * server's 428 response so a mis-configured NEXT_PUBLIC_TURNSTILE_SITE_KEY
   * on the client can still be solved using the key the server signed.
   */
  siteKey?: string | null;
};
...
export const Captcha = forwardRef<any, Props>(
  ({ validationError, siteKey: siteKeyOverride, ...props }, ref) => {
