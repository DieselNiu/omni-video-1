 ...
 *
 * The balance update is atomic (`UPDATE ... WHERE current_credits >= amount`)
 * and the whole flow runs inside a DB transaction with `SELECT ... FOR UPDATE`
 * on the FIFO rows, so concurrent calls cannot overspend.
 *
 * Throws `InsufficientCreditsError` when the balance is below `amount`.
 */
export async function consumeCredits({
  ...
  await db.transaction(async (tx) => {
    // Atomic conditional decrement — if no row matches (user has no credit
    // row, or balance < amount), the update affects 0 rows and we bail.
    const updated = await tx
      ...
      .where(
        and(
          ...
          gte(userCredit.currentCredits, amount)
        )
      )
      .returning({ currentCredits: userCredit.currentCredits });
    ...
    // FIFO consumption: lock & consume the earliest unexpired buckets first.
    ...
    // Write the USAGE log inside the same transaction.
