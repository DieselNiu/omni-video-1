...
export interface Price {
  ...
  credits?: Credits;                 // Per-price credits override (e.g. monthly vs yearly differ); falls back to plan.credits
}
...
export interface PricePlan {
  ...
  bestValue?: boolean;               // Whether to mark this plan as best value in UI
  disabled?: boolean;                // Whether to disable this plan in UI
  credits?: Credits;                 // Credits configuration for this plan
  tiers?: PricePlanTier[];           // Tiered pricing options (e.g., different credit amounts at different prices)
}

/**
 * Tiered pricing option within a plan
 */
export type PlanTier = PricePlanTier; // alias used by some components

export interface PricePlanTier {
  id: string;
  prices: Price[];
  credits: Credits;
  name?: string;
